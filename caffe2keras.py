...
class Model2Keras(object):
    ...
    def save_weights(self, h5py_save_dir=None):
        ...
        params_index = 0
        params = net.params.values()
        f.attrs['nb_layers'] = len(self.kerasmodel.layers)
        for i, layer in enumerate(self.kerasmodel.layers):
            ...
            if layer.get_config()['name'] == 'Convolution2D':
                g.attrs['nb_params'] = 2
                # w weights
                w_param_dset = g.create_dataset('param_0', params[params_index][0].data.shape, dtype=params[params_index][0].data.dtype)
                ...
                # b weihts
                b_param_dset = g.create_dataset('param_1', params[params_index][1].data.shape, dtype=params[params_index][1].data.dtype)
                ...
                g.attrs['nb_params'] = 2
                # w
                w_param_dset = g.create_dataset('param_0', params[params_index][0].data.T.shape, dtype=params[params_index][0].data.T.dtype)
...
class Proto2Keras(object):
    def __init__(self, prototext=None):
        if not available:
            raise RuntimeError('CaffeFunction is not supported on Python 3')

        if not prototext:
            raise RuntimeError('protext must specified')

        self.model = Sequential()
        net = caffe_pb2.NetParameter()

        google.protobuf.text_format.Merge(open(prototext).read(), net)
